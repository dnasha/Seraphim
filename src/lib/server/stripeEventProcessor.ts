...
import { recordTrialUse } from '@/lib/server/trialEligibility';
...
async function handleCheckoutCompleted(session: Stripe.Checkout.Session) {
  ...
  if (session.mode === 'payment' && priceKey === 'angel') {
    ...
    // Fulfillment snapshots the old subscription atomically before clearing the
    // profile. A retry after a crash must still be able to cancel that subscription.
    const { data: purchase, error: purchaseError } = await supabaseAdmin.from('angel_purchases')
      .select('replaced_subscription_id').eq('stripe_payment_intent_id', session.payment_intent).single();
    if (purchaseError || !purchase) throw purchaseError ?? new Error('angel_purchase_missing');
    if (purchase.replaced_subscription_id) {
      try {
        await stripe.subscriptions.cancel(purchase.replaced_subscription_id);
      } catch (error) {
        if (!(error instanceof Stripe.errors.StripeInvalidRequestError && error.code === 'resource_missing')) throw error;
      }
    }
    // The fulfillment RPC owns entitlement state, including pre-arriving reversals.
...
async function syncSubscription(userId: string, subscription: Stripe.Subscription) {
  const customerId = getCustomerId(subscription.customer);
  if (customerId && subscription.trial_start != null) {
  ...
  if (isEntitled && tier === 'free') throw new Error('unrecognized_subscription_price');
