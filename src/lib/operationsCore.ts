...
export function serverDiagnostic(code: string, correlationId?: string | null) {
  if (typeof process !== 'undefined' && process.stderr) {
    process.stderr.write(`[seraphim:${code}]${correlationId ? ` correlation=${correlationId}` : ''}\n`);
  }
}

export function createOperationsRecorder(client: SupabaseClient) {
  async function recordMetric(input: {
    ...
    const { error } = await client.rpc('increment_service_metric', {
  ...
  async function recordIncident(input: {
    ...
    const { error } = await client.rpc('upsert_operational_incident', {
  ...
  async function recoverIncident(dedupKey: string) {
  ...
  return { recordMetric, recordIncident, recoverIncident };
}
