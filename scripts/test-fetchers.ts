...
import { fetchGNews } from '../src/scraper/fetchers/gnews';
...
async function run() {
    ...
    const rssPromises = RSS_SOURCES.map(s => testSource(s.name, 'rss', () => fetchSingleFeed(s)));
    ...
    const tgPromises = TELEGRAM_CHANNELS.map(s => testSource(s.name, 'telegram', () => scrapeTelegramChannel(s)));
