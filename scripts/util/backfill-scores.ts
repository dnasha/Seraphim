...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
    console.error('Missing environment variables. Ensure SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are set.');
    process.exit(1);
}

interface ExtendedDbEventSource extends DbEventSource {
...
async function backfillScores() {
    ...
    console.log(`[backfill] Total events to process: ${totalEvents?.toLocaleString()}`);
    ...
    while (true) {
        ...
        console.log(`[backfill] Progress: ${progress}% | Processed: ${totalProcessed.toLocaleString()} | Speed: ${itemsPerSec} items/s`);
    }
